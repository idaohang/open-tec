#pragma ident "$Id: xRinexMet.cpp 3140 2012-06-18 15:03:02Z susancummins $"
...
void xRinexMet :: bitsAsStringTest (void)
{
	...
	CPPUNIT_ASSERT_EQUAL(a,RinexMetHeader.bitsAsString(RinexMetHeader.validVersion));
	CPPUNIT_ASSERT_EQUAL(b,RinexMetHeader.bitsAsString(RinexMetHeader.validRunBy));
	CPPUNIT_ASSERT_EQUAL(c,RinexMetHeader.bitsAsString(RinexMetHeader.validComment));
	CPPUNIT_ASSERT_EQUAL(d,RinexMetHeader.bitsAsString(RinexMetHeader.validMarkerName));
	CPPUNIT_ASSERT_EQUAL(e,RinexMetHeader.bitsAsString(RinexMetHeader.validMarkerNumber));
	CPPUNIT_ASSERT_EQUAL(f,RinexMetHeader.bitsAsString(RinexMetHeader.validObsType));
	CPPUNIT_ASSERT_EQUAL(g,RinexMetHeader.bitsAsString(RinexMetHeader.validSensorType));
	CPPUNIT_ASSERT_EQUAL(h,RinexMetHeader.bitsAsString(RinexMetHeader.validSensorPos));
	CPPUNIT_ASSERT_EQUAL(i,RinexMetHeader.bitsAsString(RinexMetHeader.validEoH));
	//Defult Case
...
void xRinexMet :: bitStringTest(void)
{
	...
	CPPUNIT_ASSERT_EQUAL(a, RinexMetHeader.bitString(RinexMetHeader.validVersion,'\"',sep));
...
void xRinexMet :: hardCodeTest (void)
{
	...
	CPPUNIT_ASSERT_EQUAL((gpstk::CommonTime)TimeGuess,RinexMetData.time);
	...
	CPPUNIT_ASSERT_EQUAL((gpstk::CommonTime)TimeGuess2,RinexMetData.time);
	...
	CPPUNIT_ASSERT(fileEqualTest((char*)"Logs/408_110a.04m",(char*)"Logs/Output.txt"));
...
void xRinexMet :: continuationTest (void)
{
	...
	try
	{
		...
		CPPUNIT_ASSERT(fileEqualTest((char*)"Logs/ContLines10.04m",(char*)"Logs/OutputCont.txt"));
	}
	catch (gpstk::Exception& e)
	{
		cout << e;
