#pragma ident "$Id: ConfDataItem.hpp 3149 2012-06-20 16:22:19Z prestonherrmann $"
...
namespace gpstk
{
   ...
   public:
      ...
      std::string format(const DayTime& d)
      {
         return StringUtils::asString(d.year()) + " "
              + StringUtils::asString(d.month()) + " "
              + StringUtils::asString(d.day()) + " "
              + StringUtils::asString(d.hour()) + " "
              + StringUtils::asString(d.minute()) + " "
              + StringUtils::asString(d.second(),3);
      }
      ...
      void parse(const std::string& in,DayTime& out)
      {
         int year,month,day,hour,minute; double second;
         istringstream s(in);
         s >> year >> month >> day >> hour >> minute >> second;    
         out.setYMDHMS(year,month,day,hour,minute,second);
      }
